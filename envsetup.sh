...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the FLEX github
        T=$(gettop)
        pushd $T > /dev/null
...
function eat()
{
    ...
    if (adb shell getprop ro.flex.device | grep -q "$FLEX_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.flex.device | grep -q "$FLEX_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/flex/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
