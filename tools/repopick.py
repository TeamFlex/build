...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
    ...
    patches from TeamFlex's Gerrit instance.
...
def fetch_query(query):
    url = 'http://198.199.71.201:3539/changes/?q=%s' % query
    if args.verbose:
...
for changeps in args.change_number:
    ...
    #   )]}'
    #   [ ... valid JSON ... ]
    url = 'http://198.199.71.201:3539/changes/?q={change}&o={query_revision}&o=CURRENT_COMMIT&pp=0'.format(change=change, query_revision=query_revision)
    if args.verbose:
